...
void DepthNumTT::store(uint64_t key, SearchResult result, EntryType type, int depth, int ageOnStarted, int ageCurrent, uint64_t subtreeSize, int irreversibleNumCurrent) {
    ...
    if ((entry.key == 0 || entry.ext < irreversibleNumber || entry.depth <= depth) && !result.bestLine.empty()) {
...
bool DepthNumTT::retrieve(uint64_t key, TTEntry& entry, bool retrieveOnlyExact) {
    //size_t index = key % TTsize;
    size_t index = key & (TTsize - 1);
